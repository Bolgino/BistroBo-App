...
// =========================================================================
// MOTORE REPARTI EXTRA (AUTO-INSTALLANTE E INDIPENDENTE)
// =========================================================================

document.addEventListener("DOMContentLoaded", () => {
    
    // 1. Inizializzazione Bottoni Impostazioni in Admin e Chat
    ['extra1', 'extra2', 'extra3'].forEach(r => {
        const R = r.charAt(0).toUpperCase() + r.slice(1);
        if(typeof setupToggleButton === "function") {
            setupToggleButton(`toggle${R}Btn`, `impostazioni/${r}`);
            setupToggleButton(`toggleNuoveInAlto${R}Btn`, `impostazioni/nuoveInAlto${R}`);
            setupToggleButton(`toggleMagazzino${R}Btn`, `impostazioni/mostraMagazzino${R}`);
            setupToggleButton(`toggleMenu${R}Btn`, `impostazioni/mostraMenu${R}`);
        }
        if(typeof ascoltaChat === "function") ascoltaChat(r, `chatContainer${R}`);
    });

    // 2. Modifica invisibile per mostrare le nuove schermate HTML
    if (typeof window.mostraSchermata === "function") {
        const originalMostra = window.mostraSchermata;
        window.mostraSchermata = function(ruolo) {
            originalMostra(ruolo);
            // Nasconde gli extra di default
            ['extra1', 'extra2', 'extra3'].forEach(r => {
                const div = document.getElementById(r + 'Div');
                if(div) div.classList.add('hidden');
            });
            // Mostra se selezionato
            if(['extra1', 'extra2', 'extra3'].includes(ruolo)) {
                document.getElementById(ruolo + 'Div').classList.remove('hidden');
                
                // Aggiorna il Titolo H2 della pagina
                const R = ruolo.charAt(0).toUpperCase() + ruolo.slice(1);
                const mainTitle = document.getElementById(`mainTitle${R}`);
                const nomeVero = window.nomiRepartiExtra ? window.nomiRepartiExtra[ruolo] : ruolo;
                if(mainTitle) mainTitle.innerText = nomeVero || ruolo;
            }
        };
    }

    // 3. Aggiunta dei "Nomi Personalizzati" ai menu a tendina (per creare i Piatti/Ingredienti)
    function aggiornaSelectCategorieAdmin() {
        const selectIds = ['categoriaPiatto', 'editCategoriaPiatto', 'categoriaIngrediente', 'editCategoriaIngrediente'];
        selectIds.forEach(id => {
            const select = document.getElementById(id);
            if(select) {
                // Rimuove opzioni "extra" vecchie
                Array.from(select.options).forEach(opt => {
                    if(opt.value.startsWith('extra')) select.removeChild(opt);
                });
                
                // Inserisce quelle con il nome giusto SE sono accese e configurate
                ['extra1', 'extra2', 'extra3'].forEach(r => {
                    if(window.impostazioni && window.impostazioni[r] && window.nomiRepartiExtra && window.nomiRepartiExtra[r]) {
                        const opt = document.createElement('option');
                        opt.value = r;
                        opt.text = window.nomiRepartiExtra[r];
    ...
    // Ascolta le impostazioni globali per nascondere/mostrare le preferenze extra
    db.ref('impostazioni').on('value', snap => {
        const imp = snap.val() || {};
        window.impostazioni = imp;
        ['extra1', 'extra2', 'extra3'].forEach(r => {
            const div = document.getElementById(`${r}DependentSettings`);
            if(div) div.style.display = imp[r] ? 'block' : 'none';
        });
        setTimeout(aggiornaSelectCategorieAdmin, 300);
    });

    db.ref("impostazioni/nomiRepartiExtra").on("value", () => setTimeout(aggiornaSelectCategorieAdmin, 300));
});

// =========================================================================
// I 3 LETTORI DATABASE INDIPENDENTI PER GLI EXTRA
// =========================================================================

// LETTORE COMANDE
db.ref("comande").on("value", snap => {
    if(!window.renderComandeList) return;
    
    let comande = {
        extra1: { daFare: [], storico: [] },
        extra2: { daFare: [], storico: [] },
        extra3: { daFare: [], storico: [] }
    };
    ...
    snap.forEach(child => {
        ...
        ['extra1', 'extra2', 'extra3'].forEach(r => {
            let piattiReparto = c.piatti.filter(p => p.categoria === r);
            if(piattiReparto.length > 0) {
                let clone = {...c, id: id, piatti: piattiReparto};
                const R = r.charAt(0).toUpperCase() + r.slice(1);
                if(clone[`completata${R}`]) {
                    comande[r].storico.push(clone);
                } else {
                    comande[r].daFare.push(clone);
                }
            }
        });
    });
    ...
    ['extra1', 'extra2', 'extra3'].forEach(r => {
        const R = r.charAt(0).toUpperCase() + r.slice(1);
        const imp = window.impostazioni || {};
        const nuoveInAlto = imp[`nuoveInAlto${R}`];
        
        comande[r].daFare.sort((a,b) => nuoveInAlto ? b.timestamp - a.timestamp : a.timestamp - b.timestamp);
        comande[r].storico.sort((a,b) => b.timestamp - a.timestamp); // Storico sempre in fondo
        
        renderComandeList(comande[r].daFare, `da${R}ComandeContainer`, r);
        renderComandeList(comande[r].storico, `storico${R}ComandeContainer`, r);
        
        const cont = document.getElementById(`conteggio${R}`);
        if(cont) cont.innerText = comande[r].daFare.length;
    });
});

// LETTORE MENU PIATTI
db.ref("menu").on("value", snap => {
    if(!window.renderPiatti) return;
    let pExtra = { extra1: [], extra2: [], extra3: [] };
    
    snap.forEach(child => {
        const p = {...child.val(), id: child.key};
        if(pExtra[p.categoria]) pExtra[p.categoria].push(p);
    });
    
    ['extra1', 'extra2', 'extra3'].forEach(r => {
        const R = r.charAt(0).toUpperCase() + r.slice(1);
        renderPiatti(pExtra[r], `menu${R}Container`, r);
    });
});

// LETTORE INGREDIENTI / MAGAZZINO
db.ref("ingredienti").on("value", snap => {
    if(!window.renderIngredienti) return;
    let iExtra = { extra1: [], extra2: [], extra3: [] };
    
    snap.forEach(child => {
        const i = {...child.val(), id: child.key};
        if(iExtra[i.categoria]) iExtra[i.categoria].push(i);
    });
    
    ['extra1', 'extra2', 'extra3'].forEach(r => {
        const R = r.charAt(0).toUpperCase() + r.slice(1);
        renderIngredienti(iExtra[r], `ingredienti${R}Container`, r);
    });
});

// HOOK PER IL COMPLETAMENTO COMANDE
if(typeof window.completaComanda === "function") {
    const orgCompleta = window.completaComanda;
    window.completaComanda = async function(id, ruolo) {
        if(['extra1','extra2','extra3'].includes(ruolo)) {
            let update = {};
            update[`completata${ruolo.charAt(0).toUpperCase() + ruolo.slice(1)}`] = true;
            await db.ref(`comande/${id}`).update(update);
        } else {
            orgCompleta(id, ruolo);
        }
    };
}

// HOOK PER IL RIPRISTINO COMANDE
if(typeof window.ripristinaComanda === "function") {
    const orgRipristina = window.ripristinaComanda;
    window.ripristinaComanda = async function(id, ruolo) {
        if(['extra1','extra2','extra3'].includes(ruolo)) {
            let update = {};
            update[`completata${ruolo.charAt(0).toUpperCase() + ruolo.slice(1)}`] = false;
            await db.ref(`comande/${id}`).update(update);
        } else {
            orgRipristina(id, ruolo);
        }
    };
}
