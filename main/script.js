...
// =========================================================================
// MOTORE DI ATTIVAZIONE E SINCRONIZZAZIONE PROFILI EXTRA
// =========================================================================

// 1. GESTIONE BOTTONI ON/OFF IN ADMIN
['extra1', 'extra2', 'extra3'].forEach(ruolo => {
    const btnToggle = document.getElementById(`toggle${ruolo.charAt(0).toUpperCase() + ruolo.slice(1)}Btn`);
    if(btnToggle) {
        btnToggle.onclick = async () => {
            if(btnToggle.disabled) return; // Se non ha nome, non fa nulla
            const isAttualmenteOn = btnToggle.innerText.includes("ON");
            // Inverte lo stato su Firebase
            await db.ref(`impostazioni/${ruolo}`).set(!isAttualmenteOn);
            if(typeof notify === "function") notify(`Reparto ${isAttualmenteOn ? 'disattivato' : 'attivato'}!`, "success");
        };
    }
});

// 2. AGGIORNAMENTO COLORI E STATI ON/OFF DAL DATABASE
db.ref("impostazioni").on("value", snap => {
    const imp = snap.val() || {};
    ['extra1', 'extra2', 'extra3'].forEach(ruolo => {
        const btnToggle = document.getElementById(`toggle${ruolo.charAt(0).toUpperCase() + ruolo.slice(1)}Btn`);
        const divDipendenti = document.getElementById(`${ruolo}DependentSettings`);
        
        if(btnToggle) {
            const isAttivo = imp[ruolo] === true;
            btnToggle.innerText = isAttivo ? "ON" : "OFF";
            btnToggle.style.backgroundColor = isAttivo ? "#4CAF50" : "#f44336";
            btnToggle.style.color = "white";
            
            // Mostra o nascondi le impostazioni aggiuntive del reparto
            if(divDipendenti) {
                divDipendenti.style.display = isAttivo ? "block" : "none";
            }
        }
    });
});

// 3. INIEZIONE DINAMICA NEI MENU A TENDINA E NELLA CASSA
window.aggiornaInterfacceExtra = function() {
    // A. AGGIUNGIAMO LE OPZIONI AI MENU A TENDINA (Pannello Admin)
    const idTendine = [
        'piattoCategoria', 
        'ingredienteCategoria', 
        'modificaCategoriaPiatto', 
        'modificaCategoriaIngrediente'
    ];

    idTendine.forEach(id => {
        const select = document.getElementById(id);
        if(!select) return;

        // Pulizia vecchie opzioni extra per evitare duplicati
        Array.from(select.options).forEach(opt => {
            if(opt.value.startsWith('extra')) opt.remove();
        });

        // Inserimento nuove opzioni
        ['extra1', 'extra2', 'extra3'].forEach(ruolo => {
            const nomePersonale = window.nomiRepartiExtra[ruolo];
            // Mostra nel menu a tendina solo se il reparto ha un nome!
            if(nomePersonale && nomePersonale.trim() !== "") {
                const opt = document.createElement('option');
                opt.value = ruolo;
                opt.innerText = nomePersonale;
                select.appendChild(opt);
            }
        });
    });

    // B. AGGIUNGIAMO I CHECKBOX DESTINAZIONE NOTE (In Cassa)
    // Cerchiamo il contenitore delle note
    const containerNote = document.getElementById('destinazioniNotaContainer'); 
    if(containerNote) {
        // Pulizia vecchi bottoni extra
        const vecchiBottoni = containerNote.querySelectorAll('.btn-destinazione-extra');
        vecchiBottoni.forEach(b => b.remove());

        // Creazione nuovi bottoni identici a quelli nativi
        ['extra1', 'extra2', 'extra3'].forEach(ruolo => {
            const nomePersonale = window.nomiRepartiExtra[ruolo];
            if(nomePersonale && nomePersonale.trim() !== "") {
                const label = document.createElement('label');
                label.className = 'btn-destinazione btn-destinazione-extra';
                ...
                label.innerHTML = `
                    <input type="checkbox" value="${ruolo}" class="destinazione-checkbox" style="transform: scale(1.5); cursor:pointer;">
                    <span style="font-weight:bold; font-size:1.1em; color:#333;">${nomePersonale}</span>
                `;
                
                // Effetto grafico quando ci clicchi sopra (identico a BistroBò nativo)
                ...
                containerNote.appendChild(label);
            }
        });
    }
};

// 4. AGGANCIO L'INIEZIONE AL CAMBIAMENTO DEI NOMI
// Sovrascriviamo leggermente il listener dei nomi creato prima per fargli lanciare l'aggiornamento grafico
db.ref("impostazioni/nomiRepartiExtra").on("value", snap => {
    const dati = snap.val() || {};
    window.nomiRepartiExtra.extra1 = dati.extra1 || "";
    window.nomiRepartiExtra.extra2 = dati.extra2 || "";
    ...
    // Aggiorna l'admin (titoli e bottoni)
    if(typeof aggiornaUIExtra === "function") {
        aggiornaUIExtra('extra1', window.nomiRepartiExtra.extra1);
        aggiornaUIExtra('extra2', window.nomiRepartiExtra.extra2);
        aggiornaUIExtra('extra3', window.nomiRepartiExtra.extra3);
    }

    // Inietta i nomi ovunque nell'app!
    setTimeout(window.aggiornaInterfacceExtra, 500); 
});
